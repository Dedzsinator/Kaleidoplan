...
const getEventById = async (req, res, next) => {
    try {
      ...
      let eventId;
      try {
        eventId = mongoose.Types.ObjectId.isValid(id) 
...
const getEventInterests = async (req, res, next) => {
  try {
    ...
    const interests = await EventInterest.find({ eventId })
      .select('userId userName userEmail reminderFrequency createdAt lastReminded');
