...
const WebAudio: WebAudioAPI = {
    ...
    Sound: {
        createAsync: async (source: string | { uri: string }) => {
            ...
            return {
                sound: {
                    playAsync: async (): Promise<void> => { void htmlAudio.play(); },
                    pauseAsync: async (): Promise<void> => { htmlAudio.pause(); },
                    unloadAsync: async (): Promise<void> => { htmlAudio.src = ''; },
...
const initializeSpotify = async (retries = 3): Promise<boolean> => {
    for (let i = 0; i < retries; i++) {
        try {
            ...
            console.warn(`Spotify authentication failed (attempt ${i + 1}/${retries}):`,
...
const testApiConnection = async (): Promise<boolean> => {
    ...
    try {
        ...
        console.warn('API connection failed, will use mock data:',
            error instanceof Error ? error.message : error);
        return false;
    }
};