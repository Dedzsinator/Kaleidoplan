  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      try {
        ...
        const mockLoginActivity = Array(30)
          .fill(0)
          .map((_, i) => {
            const date = new Date();
            ...
            datasets: [
              {
                ...
                backgroundColor: ['rgba(66, 133, 244, 0.7)', 'rgba(52, 168, 83, 0.7)', 'rgba(251, 188, 4, 0.7)'],
              },
            ],
          },
          options: {
            ...
            datasets: [
              {
                ...
                backgroundColor: ['rgba(52, 168, 83, 0.7)', 'rgba(66, 133, 244, 0.7)', 'rgba(234, 67, 53, 0.7)'],
              },
            ],
          },
          options: {
    ...
    <div className="analytics-tab">
      ...
      <div className="kpi-summary">
        <div className="kpi-card">
          ...
          <div className="kpi-trend">+{stats?.users.new || 0} new (last 7 days)</div>
        </div>

        <div className="kpi-card">
          <h3>Total Events</h3>
          <div className="kpi-value">{stats?.events.total || 0}</div>
          <div className="kpi-trend">{stats?.events.upcoming || 0} upcoming</div>
        </div>

        <div className="kpi-card">
          <h3>Tasks</h3>
          <div className="kpi-value">{stats?.tasks.total || 0}</div>
          <div className="kpi-trend">{stats?.tasks.completionRate || 0}% completion rate</div>
        </div>
              ...
              <div key={user._id} className="active-user-card">
                <div className="user-avatar">
                  {user.photoURL ? (
                    ...
                    <div className="default-avatar">{(user.displayName || user.email || '?')[0].toUpperCase()}</div>
                  )}
